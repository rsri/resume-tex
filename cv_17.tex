\documentclass[a4paper]{srika-resume} % Use US Letter paper, change to a4paper for A4 

\begin{document}

%----------------------------------------------------------------------------------------
%	TITLE SECTION
%----------------------------------------------------------------------------------------

% \lastupdated % Print the Last Updated text at the top right

\namesection{SRIKANTH}{R}{ % Your name
{Android/Java developer} \\
\href{mailto:srikanthr.0402@gmail.com}{srikanthr.0402@gmail.com} | +91-9788536910 % Your contact information
}

%----------------------------------------------------------------------------------------
%	LEFT COLUMN
%----------------------------------------------------------------------------------------

\begin{minipage}[t]{0.35\textwidth} % The left column takes up 35% of the text width of the page

%------------------------------------------------
% Education
%------------------------------------------------

\section{Education} 
\subsection{Karpagam college of Engineering}

\descript{BE - Computer Science and Engineering}
\location{April 2013 | Coimbatore, India \\ Cum. GPA: 8.28}

\sectionspace % Some whitespace after the section

%------------------------------------------------

\subsection{Sri Gopal Naidu Higher Secondary School}

\location{May 2009 | Coimbatore, India \\ Secondary school: 90.5\%}

\sectionspace % Some whitespace after the section

%------------------------------------------------

\subsection{ABC Matriculation Higher Secondary School}

\location{May 2007 | Coimbatore, India \\ High school: 87.7\%}

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Links
%------------------------------------------------

\section{Links} 

Github:// \href{https://github.com/rsri}{\bf rsri} \\
LinkedIn:// \href{https://www.linkedin.com/in/srikaram}{\bf srikaram} \\
Quora:// \href{https://www.quora.com/profile/Srikanth-Ramachandran-1}{\bf Srikanth-Ramachandran-1}
Hackerrank:// \href{https://www.hackerrank.com/nocomment}{\bf Srikanth R}

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Coursework
%------------------------------------------------

\section{Github projects}

\href{https://github.com/rsri/GamesOfThronesC}{\subsection {GOTC - A programming language}}

Programming language developed with Java where syntax and statements use dialogues and phrases from \textbf{Game of Thrones} show.

%------------------------------------------------

\href{https://github.com/rsri/Pic2Fro}{\subsection {Pic2Fro - Windows moviemaker for Android}}

Android app to create a short length video using photos in gallery or using camera muxed with user-picked background audio.

%------------------------------------------------

\href{https://github.com/rsri/MusicPlayer-Ola}{\subsection {Music Player}}

An Android app that streams audio from server and plays. Provides options to play/pause and to download locally.

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Skills
%------------------------------------------------

\section{Skills}

\subsection{Programming}

\location{Over 5000 lines:}
Java \textbullet{} Android \\
\location{Over 1000 lines:}
SQL-Oracle DB \textbullet{} HTML5 \textbullet{} CSS \textbullet{} Oracle ADF \textbullet{} JavaScript/jQuery \\
\location{Familiar:}
iOS (Objective-C) \textbullet{} Docker \textbullet{} Bash scripting \textbullet{} Bootstrap \\

\sectionspace % Some whitespace after the section

%----------------------------------------------------------------------------------------

\end{minipage} % The end of the left column
\hfill
%
%----------------------------------------------------------------------------------------
%	RIGHT COLUMN
%----------------------------------------------------------------------------------------
%
\begin{minipage}[t]{0.64\textwidth} % The right column takes up 64% of the text width of the page

%------------------------------------------------
% Experience
%------------------------------------------------

\section{Experience}

\runsubsection{Oracle India}
\descript{| Applications Engineer}

\location{Jul 2016 - Present | Bangalore, India}
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightitemize}
\item Working in the \textbf{\href{https://docs.oracle.com/cloud/latest/salescs_gs/OASAL/OASAL1439686.htm\#OASAL1439686}{Oracle Sales Cloud assignment engine}} team in Java with ADF and Oracle DB to develop an engine that assigns resources to business objects used by sales administrators to implement, configure, and use administrative components of the cloud service.
\item Implemented a workflow in the module where an assignment invocation would identify the matching candidate(s) for each work object that's being assigned.
\end{tightitemize}

\sectionspace % Some whitespace after the section

%------------------------------------------------

\runsubsection{Impiger technologies}
\descript{| Senior Software Development Engineer}

\location{Sep 2013 – Jun 2016 | Coimbatore, India}
\begin{tightitemize}
\item Worked in \textbf{\href{https://en.wikipedia.org/wiki/Quickoffice}{Quickoffice, an Android Officesuite application}} development team
\item Developed several Android applications adhering to full SDLC
\item Worked on internal web-based project named Resourcefully using HTML5 and CSS3 with JQuery incorporated within Django for employee projects tracker
\end{tightitemize}

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Projects
%------------------------------------------------

\section{Projects}

\runsubsection{QUICKOFFICE}
\descript{| GOOGLE DOCS, SHEETS, SLIDES}

\location{Android, XML, Ant}
An OfficeSuite application that later got integrated into Google Apps that allows to
create, access and edit Word, Excel, and PowerPoint documents.\\
Implemented the feature to apply, view and remove \textbf{Styles} in Word documents. \\
Implemented several \textbf{Formulas} that can be inserted and used in Excel documents.

\sectionspace % Some whitespace after the section

%------------------------------------------------

\runsubsection{Resourcefully}

\location{HTML5, CSS, JavaScript, jQuery, Django}
An internal project used by managers for resource allocation along with management of projects, vacations and employees.  \\
Implemented the feature to add, edit and remove employees and eventually allocate them to a project depending on the manager the resource is under.

\sectionspace % Some whitespace after the section

%------------------------------------------------

\runsubsection{Timetracker}

\location{Android, Gradle, Firebase}
As an extension of Resourcefully, developed an entire Android app used by the employees to update their timesheets, their tasks and submit them for review. 

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Extra-curricular activities
%------------------------------------------------

\section{Extra-curricular activities} 
\begin{tightitemizespaced}
\item Won 'first' prize in 'Lateral and Logical thinking' event in an inter-school competition conducted at G.Ramaswamy Naidu Matriculation Higher Secondary School
\item Won 'first' prize intra-college 'Chess' tournament conducted at Karpagam college of Engineering
\item Acted as a student coordinator in organizing “Ripple'12 \& Ripple'13, National Level Technical Symposiums” conducted at Karpagam college of Engineering
\end{tightitemizespaced}

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Extra-curricular activities
%------------------------------------------------

\section{Hobbies} 
\begin{tightitemizespaced}
\item Playing chess
\item Watching television series
\item Reading books
\item Badminton
\end{tightitemizespaced}

\sectionspace % Some whitespace after the section

%----------------------------------------------------------------------------------------

\end{minipage} % The end of the right column

%----------------------------------------------------------------------------------------
%	SECOND PAGE (EXAMPLE)
%----------------------------------------------------------------------------------------

%\newpage % Start a new page

%\begin{minipage}[t]{0.33\textwidth} % The left column takes up 33% of the text width of the page

%\section{Example Section}

%\end{minipage} % The end of the left column
%\hfill
%\begin{minipage}[t]{0.66\textwidth} % The right column takes up 66% of the text width of the page

%\section{Example Section 2}

%\end{minipage} % The end of the right column

%----------------------------------------------------------------------------------------

\end{document}